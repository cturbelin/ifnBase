...
We recommand an workspace organization (see [workspace](workspace.html))

## Configuration

Several configuration information should be provided in order the package works correctly

Configuration is holded in a list, stored in R `options()` entry named 'ifn'
...
We provided several function allowing to use files from a location outside from a given project. They assume an external location (called "share" as its is intented to be shared accross several projects).
...
All further ouput (using my.path()) will be done into a "web" subdirectory of the current ouput path. If the "web" directory does not exists, it will be created.

To know the path of a given file in the ouput path (to load the file or to output data), use the file name wrapped by the
my.path() function:
```r
  my.path('myfile.csv')
```
...
A week number is reprensented as yearoftheweek * 100 + weeknumber (called yearweek number)
...
* `WeekStamp(yw)` : give the continuous index of the yearweek number (enabling easy arithmetic operations on week numbers)

## Geographic levels

Package provivdes a set of functions to handle geographic levels (to describe a way a territory, let's say a country, is divided in several areas in a hierarchical ways.). You can have several levels (country, regions, ...). Each level is divided into areas, which can be divided in sub-area in the lower level.

The only assumption made is an area can only belongs to a single area on the upper level (it doesnt handle overlap). But it can handle several level hierarchies.
...
* select_columns() : extract columns from data.frame() and warn if does not exist

## Loading libraries functions

To load function of a library in share/ you have to use the `share.lib` function.

```
  share.lib('survey') # load survey functions
  share.lib(c('survey','syndrom','i18n')) # load several
```
